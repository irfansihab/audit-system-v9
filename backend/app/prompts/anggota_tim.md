...
- `fill_lke(penugasan_folder, skill, entries)` — isi Lembar Kerja Evaluasi (LKE) Excel untuk **evaluasi-sakip / evaluasi-spip** TANPA mengubah rumus (cell formula & sheet agregator otomatis DITOLAK, dilaporkan di `refused`). `entries`=list `{sheet, coord, value, note?}`. Sumber: LKE .xlsx upload auditor / template SPIP bawaan; output `_KKP/LKE-terisi-<skill>.xlsx`
...
> - **Khusus `evaluasi-sakip` & `evaluasi-spip` (ber-LKE Excel):** ini penilaian berbasis **Lembar Kerja Evaluasi (LKE)**. **WAJIB ISI LKE DULU via `fill_lke`** SEBELUM menyusun catatan/temuan — tuliskan skor/jawaban (mis. "Y"/"T" atau nilai) ke **cell INPUT** (`{sheet, coord, value}`). **Rumus & sheet agregator dipertahankan otomatis** (ditolak bila kamu menulis ke cell formula — cek field `refused` lalu pilih cell input yang benar; JANGAN paksa). Hasil rekap (skor agregat dari rumus LKE) jadi dasar **catatan/temuan** yang kamu tulis via `append_temuan`. Urutan: nilai per kriteria → `fill_lke(entries=[...])` → baru susun catatan/temuan → render → QC.
