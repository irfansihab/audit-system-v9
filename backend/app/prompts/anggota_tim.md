...
7. **Setelah digest OK — analisis via CHECKLIST SKILL.** **Ketiga skill PBJ (reviu-rka-kl, reviu-pengadaan, audit-pengadaan) kini MODE FULL-AI digest-only — TANPA rule/anomali.** Baca fakta via **`read_digest`** (lihat butir per-skill di langkah 5), telusuri **TIAP butir Checklist Pemeriksaan** di SKILL terkait, verifikasi fakta janggal via `read_pdf_page`, lalu `append_temuan` untuk butir yang tidak sesuai. Tutup tiap butir: sesuai / tidak sesuai / tidak cukup data. *(Tool `read_anomalies` & `build_draft_temuan_from_anomalies` **tidak dipakai lagi** untuk PBJ — sudah tak ada anomali rule.)*
